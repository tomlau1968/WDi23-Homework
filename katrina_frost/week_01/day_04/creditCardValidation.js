...
const validateCreditCard = function(creditCardNum) {
  let ccNumberNoDashes = " "; //Remove dashes from credit card number
 ...
 // All of the digits must be numbers
  for(let i = 0; i < ccNumberNoDashes.length; i++) {
    if(!Number.isInteger(Number(ccNumberNoDashes[i]))) {
      // isInteger is a wole number or not
      return false;
  }
 }
   // The credit card number must have at least two different digits (i.e. all of the digits cannot be the same)
let obj = {};
  for(let i = 0; i < ccNumberNoDashes.length; i++) {
